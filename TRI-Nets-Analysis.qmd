...
Identifying egregious pollutors wrt RSEI Hazard within each industry/NAICS code with at least 5 facilities reporting: 
...
Criteria 1: top 10% of pollutors 
...
egregious2 = function(data = TRI_NETS,year){
  ...
  for(i in valid_NAICS){
    ...
    top_pollutors = which(cumulative_pollution_sum <= (all_facilities_releases*0.9))
    
    egregious = industry_yr_data_ordered$Duns_TRIFID[c(1:(length(top_pollutors)+1))]
...
Comparing egregious pollutors across operationalizations: 

First, label all facilities which are egregious pollutors

```{r}
label_egregious_facilities = function(data,year,operationalization){
  data_label = data %>% filter(Year == year)
  data_label$egregious_pollutor = FALSE
  for (i in 1:length(operationalization)){
    ...
    data_label$egregious_pollutor[idx] = TRUE
  }  
...
```{r}
#Number of facilities categorized as egregious pollutors
sum(TRI_NETS20_label1$egregious_pollutor)

#Percent of facilities categorized as egregious pollutors
100*(sum(TRI_NETS20_label1$egregious_pollutor)/length(TRI_NETS20_label1$egregious_pollutor))

#Number of industries with egregious pollutors
length(unique(TRI_NETS20_label1$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label1$egregious_pollutor == TRUE)]))

#Percent of industries with egregious pollutors
100*(length(unique(TRI_NETS20_label1$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label1$egregious_pollutor == TRUE)]))/length(valid_NAICS))

#Percent of total annual pollution contributed by egregious pollutors
#Option 1 (simplest option)
total_annual_pollution = sum(TRI_NETS20_label1$RSEI_Hazard,na.rm = TRUE)

total_egregious_pollution = sum(unique(TRI_NETS20_label1$RSEI_Hazard[which(TRI_NETS20_label1$egregious_pollutor == TRUE)]))
...
#   egregious_pollution = sum(data$RSEI_Hazard[which(data$egregious_pollutor == TRUE)],na.rm = TRUE)
#   
...
#Median RSEI among egregious pollutors 
median(TRI_NETS20_label1$RSEI_Hazard[which(TRI_NETS20_label1$egregious_pollutor == TRUE)])

#Minimum RSEI among egregious pollutors 
min(TRI_NETS20_label1$RSEI_Hazard[which(TRI_NETS20_label1$egregious_pollutor == TRUE)])

#Maximum RSEI among egregious pollutors 
format(max(TRI_NETS20_label1$RSEI_Hazard[which(TRI_NETS20_label1$egregious_pollutor == TRUE)]),scientific = FALSE)
```
...
```{r}
#Number of facilities categorized as egregious pollutors
sum(TRI_NETS20_label2$egregious_pollutor)

#Percent of facilities categorized as egregious pollutors
100*(sum(TRI_NETS20_label2$egregious_pollutor)/length(TRI_NETS20_label2$egregious_pollutor))

#Number of industries with egregious pollutors
length(unique(TRI_NETS20_label2$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label2$egregious_pollutor == TRUE)]))

#Percent of industries with egregious pollutors
100*(length(unique(TRI_NETS20_label2$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label2$egregious_pollutor == TRUE)]))/length(valid_NAICS))

#Percent of total annual pollution contributed by egregious pollutors
#Option 1 (simplest option)
total_annual_pollution = sum(TRI_NETS20_label2$RSEI_Hazard,na.rm = TRUE)

total_egregious_pollution = sum(unique(TRI_NETS20_label2$RSEI_Hazard[which(TRI_NETS20_label2$egregious_pollutor == TRUE)]),
                                na.rm = TRUE)
...
#Median RSEI among egregious pollutors 
median(TRI_NETS20_label2$RSEI_Hazard[which(TRI_NETS20_label2$egregious_pollutor == TRUE)],
       na.rm = TRUE)

#Minimum RSEI among egregious pollutors 
min(TRI_NETS20_label2$RSEI_Hazard[which(TRI_NETS20_label2$egregious_pollutor == TRUE)],
    na.rm = TRUE)

#Maximum RSEI among egregious pollutors 
format(max(TRI_NETS20_label2$RSEI_Hazard[which(TRI_NETS20_label2$egregious_pollutor == TRUE)],
...
```{r}
#Number of facilities categorized as egregious pollutors
sum(TRI_NETS20_label3$egregious_pollutor)

#Percent of facilities categorized as egregious pollutors
100*(sum(TRI_NETS20_label3$egregious_pollutor)/length(TRI_NETS20_label3$egregious_pollutor))

#Number of industries with egregious pollutors
length(unique(TRI_NETS20_label3$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label3$egregious_pollutor == TRUE)]))

#Percent of industries with egregious pollutors
100*(length(unique(TRI_NETS20_label3$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label3$egregious_pollutor == TRUE)]))/length(valid_NAICS))

#Percent of total annual pollution contributed by egregious pollutors
#Option 1 (simplest option)
total_annual_pollution = sum(TRI_NETS20_label3$RSEI_Hazard,na.rm = TRUE)

total_egregious_pollution = sum(unique(TRI_NETS20_label3$RSEI_Hazard[which(TRI_NETS20_label3$egregious_pollutor == TRUE)]))
...
#Median RSEI among egregious pollutors 
median(TRI_NETS20_label3$RSEI_Hazard[which(TRI_NETS20_label3$egregious_pollutor == TRUE)])

#Minimum RSEI among egregious pollutors 
min(TRI_NETS20_label3$RSEI_Hazard[which(TRI_NETS20_label3$egregious_pollutor == TRUE)])

#Maximum RSEI among egregious pollutors 
format(max(TRI_NETS20_label3$RSEI_Hazard[which(TRI_NETS20_label3$egregious_pollutor == TRUE)]),scientific = FALSE)
```
...
```{r}
#Number of facilities categorized as egregious pollutors
sum(TRI_NETS20_label4$egregious_pollutor)

#Percent of facilities categorized as egregious pollutors
100*(sum(TRI_NETS20_label4$egregious_pollutor)/length(TRI_NETS20_label4$egregious_pollutor))

#Number of industries with egregious pollutors
length(unique(TRI_NETS20_label4$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label4$egregious_pollutor == TRUE)]))

#Percent of industries with egregious pollutors
100*(length(unique(TRI_NETS20_label4$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label4$egregious_pollutor == TRUE)]))/length(valid_NAICS))

#Percent of total annual pollution contributed by egregious pollutors
#Option 1 (simplest option)
total_annual_pollution = sum(TRI_NETS20_label4$RSEI_Hazard,na.rm = TRUE)

total_egregious_pollution = sum(unique(TRI_NETS20_label4$RSEI_Hazard[which(TRI_NETS20_label4$egregious_pollutor == TRUE)]))
...
#Median RSEI among egregious pollutors 
median(TRI_NETS20_label4$RSEI_Hazard[which(TRI_NETS20_label4$egregious_pollutor == TRUE)])

#Minimum RSEI among egregious pollutors 
min(TRI_NETS20_label4$RSEI_Hazard[which(TRI_NETS20_label4$egregious_pollutor == TRUE)])

#Maximum RSEI among egregious pollutors 
format(max(TRI_NETS20_label4$RSEI_Hazard[which(TRI_NETS20_label4$egregious_pollutor == TRUE)]),scientific = FALSE)
```
...
```{r}
#Number of facilities categorized as egregious pollutors
sum(TRI_NETS20_label5$egregious_pollutor)

#Percent of facilities categorized as egregious pollutors
100*(sum(TRI_NETS20_label5$egregious_pollutor)/length(TRI_NETS20_label5$egregious_pollutor))

#Number of industries with egregious pollutors
length(unique(TRI_NETS20_label5$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label5$egregious_pollutor == TRUE)]))

#Percent of industries with egregious pollutors
100*(length(unique(TRI_NETS20_label5$`41. PRIMARY NAICS CODE`[which(TRI_NETS20_label5$egregious_pollutor == TRUE)]))/length(valid_NAICS))

#Percent of total annual pollution contributed by egregious pollutors
#Option 1 (simplest option)
total_annual_pollution = sum(TRI_NETS20_label5$RSEI_Hazard,na.rm = TRUE)

total_egregious_pollution = sum(unique(TRI_NETS20_label5$RSEI_Hazard[which(TRI_NETS20_label5$egregious_pollutor == TRUE)]))
...
#Median RSEI among egregious pollutors 
median(TRI_NETS20_label5$RSEI_Hazard[which(TRI_NETS20_label5$egregious_pollutor == TRUE)])

#Minimum RSEI among egregious pollutors 
min(TRI_NETS20_label5$RSEI_Hazard[which(TRI_NETS20_label5$egregious_pollutor == TRUE)])

#Maximum RSEI among egregious pollutors 
format(max(TRI_NETS20_label5$RSEI_Hazard[which(TRI_NETS20_label5$egregious_pollutor == TRUE)]),scientific = FALSE)
```
...
EP1 = TRI_NETS20_label2$TRIFID[which(TRI_NETS20_label1$egregious_pollutor 
                                     == TRUE)]
EP2 = TRI_NETS20_label2$TRIFID[which(TRI_NETS20_label2$egregious_pollutor 
                                     == TRUE)]
EP3 = TRI_NETS20_label3$TRIFID[which(TRI_NETS20_label3$egregious_pollutor 
                                     == TRUE)]
EP4 = TRI_NETS20_label4$TRIFID[which(TRI_NETS20_label4$egregious_pollutor 
                                     == TRUE)]
EP5 = TRI_NETS20_label5$TRIFID[which(TRI_NETS20_label5$egregious_pollutor 
                                     == TRUE)]
...
for(i in valid_NAICS){
  ...
  NAICS_table1[tracker,4] = sum(NAICS_data$egregious_pollutor,na.rm = TRUE)
...
# NAICS_table1 = TRI_NETS20_label1 %>% group_by(`41. PRIMARY NAICS CODE`) %>% summarize(total_pollution = sum(RSEI_Hazard,na.rm = TRUE), median_pollution = median(RSEI_Hazard,na.rm = TRUE), median_employees = median(Employees,na.rm = TRUE), num_egregious = sum(egregious_pollutor,na.rm = TRUE))
```
...
state_table = TRI_NETS20_label3 %>% group_by(`14. FACILITY STATE`) %>% summarize(num_facilities = length(unique(Duns_TRIFID)), num_EPs = sum(egregious_pollutor))
```
...
levels(train_set$egregious_pollutor) = c("No","Yes")

test_set = na.omit(test_set)
levels(test_set$egregious_pollutor) = c("No","Yes")
```
...
smote_rf_cv <- function(train_data, perc_over, perc_under, k = 5, ntree_values = 500) {
  ...
  smote_train <- smote(egregious_pollutor ~ ., data = train_data, perc.over = perc_over, perc.under = perc_under)
  ...
  rf_model <- train(egregious_pollutor ~ ., 
                    data = smote_train, 
                    method = "rf", 
...
optimal_smote <- smote(egregious_pollutor ~ ., data = a, perc.over = best_combination$perc_over, k = 5,
                       perc.under = best_combination$perc_under)
```

```{r}
optimal_smote <- smote(egregious_pollutor ~ ., data = train_set, perc.over = 10, 
                       k = 5,perc.under = 2.3)

table(train_set$egregious_pollutor)
table(optimal_smote$egregious_pollutor)
```

Fit random forest model to training data

```{r}
EP_classifier_RF = randomForest(x = optimal_smote[,1:10],y = optimal_smote$egregious_pollutor,ntree = 500,mtry=3)
EP_classifier_RF
...
```{r}
#Confusion matrix
confusion = table(test_set$egregious_pollutor,preds)
confusion
...
#Confusion matrix
confusion = table(test_set$egregious_pollutor,preds2$predictions)
confusion
...
```{r}
EP_glm = glm(egregious_pollutor ~., data = glm_data, family = "binomial")
summary(EP_glm)
...
# Fit augmented model
augmented_model <- glm(egregious_pollutor ~., family = binomial, data = glm_data)
